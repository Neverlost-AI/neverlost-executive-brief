...
## Phase 2A - Manual Workstream Command Center Specification v0.2
...
**Supersedes:** `NEVERLOST_EXECUTIVE_BRIEF_PHASE_2A_COMMAND_CENTER_SPEC_v0_1.md`  
...
- **Status** - `proposed`, `active`, `paused`, or `completed`.
...
| `status` | Text/check | Yes | `proposed`, `active`, `paused`, `completed` |
| `health` | Text/check | Yes | `on_track`, `at_risk`, `off_track` |
...
6. preserve command type, next action, due date, review date, Phase 1 fields, and timestamps not explicitly governed above; and
7. delete only the workstream row.
...
Latest meaningful update is:

`latest_status_updated_at -> updated_at -> created_at`
...
Given the owner is triaging an item, when a valid workstream is created, it is immediately available for assignment without losing the triage form.
...
Given the approved America/Denver date boundary, a non-completed workstream appears stale only when its review date is overdue or its latest meaningful update exceeds its configured calendar-day threshold.
...
Given a workstream with entries, deleting the workstream deletes no entries, removes their assignment, returns them to inbox, preserves first-triage evidence and Phase 1 fields, and makes non-archived entries visible in Needs triage.
...
- Workstream deletion preserving and returning entries to inbox.
...
- Stale detection on the threshold boundary.
...
- Workstream deletion and re-triage.
...
1. Record human approval of this v0.2 specification.
...
PHASE_2A_RECONCILED_SPECIFICATION_V0_2_READY_FOR_HUMAN_REVIEW
